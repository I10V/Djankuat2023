...
ggsave(plot = g2 , filename = 'g2.png', width = 12, height = 9)


djan_sed_all = bed_susp_2007_2023 %>% subset(year1 == 2016 | year1 == 2017 | year1 == 2023) %>% transmute(date, bsd, ssd, q, tsd = bsd + ssd) %>% na.omit()

# summary( djan_sed_all$bsd/(djan_sed_all$ssd+djan_sed_all$bsd) * 100 )
# 
# corrplot::corrplot(djan_sed_all %>% select(-date) %>% cor() , method = 'number' )
# 
# t= lm(tsd ~ q, data = djan_sed_all) 
# t
# 
# plot(y = djan_sed_all$tsd, x = djan_sed_all$q)

bed_susp_2007_2023 = bed_susp_2007_2023 %>% mutate(tsd = 0.017*exp(1.867*q))
...
ggsave(plot = g3 , filename = 'g3.png', width = 12, height = 5)
...
writexl::write_xlsx(bed_susp_2007_2023_stat, 'bed_susp_2007_2023_stat2.xlsx')
